...
# Computes the optical flow for feature points between the current and the previous frame
def optical_flow(frame, prev_frame, mask):
    gray1 = cv2.cvtColor(prev_frame, cv2.COLOR_BGR2GRAY)
    gray2 = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)

    # Finding optimal feature points using Shi-Tomasi corner detection
    corners = cv2.goodFeaturesToTrack(gray1, 15, 0.1, 10)
    new_pos, status, _ = cv2.calcOpticalFlowPyrLK(gray1, gray2, corners, None, winSize=(50, 50), maxLevel=3)
    ...
    for points_pair in zip(new_positions, old_positions):
        cv2.line(mask, points_pair[0], points_pair[1], (255, 0, 0), 2)
...
def display_text(frame, program, settings):
    ...
    if program == 4:
        clear_mask = "[R] Clear Mask"
        cv2.putText(frame, clear_mask, (490,15), font, font_scale, color, thickness)
...
def main():
    ...
    while True:
        ...
        if program == 0:
            ...
            if optical_flow_mask is None:
                optical_flow_mask = np.zeros_like(frame)
            processed_frame = optical_flow(frame.copy(), prev_frame, optical_flow_mask)
            display_text(processed_frame, program, settings)
            cv2.imshow('Original Frame', processed_frame)
            prev_frame = frame.copy()
